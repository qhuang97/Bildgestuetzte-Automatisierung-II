...
n_components = 2 #保留的主成分个数，或保留的特征个数
ipca = IncrementalPCA(n_components=n_components, batch_size=10) #增量主成分分析，batch_size每个批次要使用的样本数
X_ipca = ipca.fit_transform(X)

pca = PCA(n_components=n_components)
X_pca = pca.fit_transform(X)
#fit()计算训练集X的均值，方差，最大值，最小值,这些训练集X固有的属性。
